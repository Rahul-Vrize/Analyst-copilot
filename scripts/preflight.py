#!/usr/bin/env python
"""Pre-demo check: is every dependency reachable from THIS network?

    python scripts/preflight.py

⚠️ WHY THIS EXISTS. Azure Postgres allows connections by SOURCE IP. Moving to a
different network - a venue's wifi, a phone hotspot - gives you a new public IP,
and every database call then fails with a timeout that looks nothing like a
firewall rule. The failure surfaces as a dead corpus panel and a dead "Add
filing" button, minutes before a demo.

Each check is independent and prints what to do about it, because the useful
question is never "is it broken" but "which part, and can I still demo".
"""
...
def check_llm(settings) -> bool:
    ...
    try:
        ...
        return False
    # ⚠️ DELIBERATELY NOT PRINTING THE REPLY. The composer stage expects a
...
def check_backend() -> bool:
    ...
    for port in (8300, 8000):
        try:
            ...
            continue
    _line(WARN, "backend", "not running on :8300 or :8000 (start uvicorn if demoing)")
    return False
...
def main() -> int:
    ...
    print()
    if db and llm:
        print("Ready. Warm the pipeline before the demo by asking one question -")
