...
-- Source of truth: IMPLEMENTATION_PLAN.md §21.2
--
-- Must build from zero on an empty database: that is exactly what a grader does.
...
-- ═════════════════════════════════════════════════════════════════════════════
-- CATALOG — what the document router filters on (§14.3, §22.2)
-- ═════════════════════════════════════════════════════════════════════════════
...
-- Seeded from data/company_aliases.yaml — see IMPLEMENTATION_PLAN §22.1.
...
-- ═════════════════════════════════════════════════════════════════════════════
-- PAGES — THE retrieval unit AND the citation unit (§21.1)
...
CREATE TABLE IF NOT EXISTS pages (
    ...
    embedding      vector(1024),                -- (3) over context_header + summary.
                                                ...
                                                --     ⚠️ Changing the embedding model means
...
-- ═════════════════════════════════════════════════════════════════════════════
-- SECTION TREE — what the reasoning navigator reads (§14.2)
...
-- ═════════════════════════════════════════════════════════════════════════════
-- XBRL FACTS — a first-class fact path, not an answer path (D8 revised, §17)
...
CREATE TABLE IF NOT EXISTS query_log (
    ...
    tier            INTEGER,                    -- 1 = first pass, 2 = escalated (§20.1)
    latency_ms      INTEGER,
...
CREATE TABLE IF NOT EXISTS answer_log (
    ...
    answer_text    TEXT,
    formula        TEXT,
    formula_source TEXT,                        -- question|book|llm_proposed (§18.3 precedence)
    operands       JSONB,
    citations      JSONB,
