...
All deterministic: no network, no database, no LLM (§24.5). These are the
...
def test_book_is_also_the_concept_map(book):
    """§17.3: `operands.concepts` feeds the XBRL path directly."""
...
def test_a_disabled_gate_is_skipped_for_ablation():
    """§24.3: the ablation runner switches one gate off and measures it."""
