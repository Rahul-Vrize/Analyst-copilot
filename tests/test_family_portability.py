...
⚠️ WHY THIS MATTERS HERE, SPECIFICALLY. D4 requires two INDEPENDENT model
families at the generate->verify seam; today both verifiers are the same
gpt-5-mini, which is self-verification wearing a costume.

The intended fix (Claude Haiku as verifier B) is NOT available on this
subscription: Claude on Microsoft Foundry bills through Azure Marketplace, and
Microsoft's docs exclude "free trial" and credit-only accounts from purchasing
it. The second families that ARE reachable - Grok, DeepSeek, Llama, Mistral -
are served by the same `/openai/v1` Foundry route through the OpenAI SDK, so
they need no new adapter.
