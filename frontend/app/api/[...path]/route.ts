 ...
 *
 * ⚠️ THIS IS A ROUTE HANDLER, NOT A `rewrites()` ENTRY, AND THE DIFFERENCE BIT.
 ...
 *   • ONE SETTING. A grader sets BACKEND_URL, or nothing, and gets the
 *     documented default.
 *
...
async function proxy(request: NextRequest, path: string[]): Promise<Response> {
  ...
  for (const [key, value] of request.headers) {
    ...
    // ⚠️ `expect` MUST BE STRIPPED, AND THIS IS WHAT BROKE "ADD FILING".
