"""Ablation runner - produces the approach-note table BY SCRIPT, not by hand (§19.4).
...
The seams already exist by design, which is why this file is short:
  * gates are a LIST OF OBJECTS, so any one can be disabled (§24.3)
  * `retrieval.use_reranker` / `use_dense` are config flags
  * `build_pipeline(with_verifiers=False)` drops the LLM verifiers

⚠️ READ BOTH COLUMNS TOGETHER. A configuration that raises the score while
...
VARIANTS: list[Variant] = [
    ...
    Variant(
        "router_top8",
        "Decision E's counterfactual: top-4 and top-8 both hit 134/136, so "
