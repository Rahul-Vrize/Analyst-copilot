...
WHY THIS EXISTS: the budget is finite ($200 of Azure credit) and a full
136-question run makes 4-6 model calls per question. Running blind risks
spending the credit on a configuration that was already failing at question 3.
...
guarantee that nothing under it knows about evaluation (§25.3).

⚠️ TOKENS ARE MEASURED; DOLLARS ARE DERIVED. The per-token rates live in
...
class CostLedger:
    ...
    ⚠️ A CONTEXTVAR, NOT `threading.local`, AND THE DIFFERENCE IS LOAD-BEARING.
