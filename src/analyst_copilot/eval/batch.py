...
rate-limited deployment, and BUILD_LOG measures 60-130 s per question. Spending
...
⚠️ WHAT COUNTS AS A FAILURE IS NOT OBVIOUS, AND THE DEFAULT MATTERS.
...
(BUILD_LOG §8), so a policy of "stop unless +1" would halt on the first or
...
def stratified_sample(questions: list[GoldQuestion], n: int) -> list[GoldQuestion]:
    ...
    ⚠️ `--limit N` IS NOT A SAMPLE. The practice set is ordered by company, so
...
def render_question(
    ...
    *,
    ...
    if run.verifiers:
        ...
        # ⚠️ THE REJECTION REASON IS THE WHOLE POINT WHEN CALIBRATING.
