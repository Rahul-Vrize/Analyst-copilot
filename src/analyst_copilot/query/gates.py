"""Deterministic gates G1-G7 with exact predicates (§20.2).

Run in order, cheapest first. ANY failure => `Not found in this filing.` with
`abstain_reason` set to the gate id.

⚠️ G1 IS THE SINGLE MOST IMPORTANT PIECE OF CODE IN THE SYSTEM. It makes an
...
exactly one and measure its contribution (§24.3).
"""
...
def normalize(text: str) -> str:
    """Casefold, collapse whitespace, strip `$ , ( ) %` (§20.2).
...
class G1bValueInQuote:
    ...
    def check(self, c: AnswerCandidate) -> GateResult:
        ...
        for operand in numeric:
            # ⚠️ Check against THIS operand's own quote. Keying quotes by page
...
class G4SlotsComplete:
    ...
    it - the inverted escape hatch of §23.2.

    ⚠️ SCOPED TO ANSWERS BUILT FROM OPERANDS. A missing operand makes a computed
