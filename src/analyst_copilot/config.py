...
THIS IS THE ONLY MODULE THAT READS os.environ (IMPLEMENTATION_PLAN §24.1).
Stages receive their settings; they never fetch them. A grep for `os.environ`
outside this file is a bug, and so is a numeric literal in stage code.

Split of responsibility (§24.4):
...
def _resolve_model(stage: str, raw: dict[str, Any], env: dict[str, str]) -> ModelSettings:
    ...
    latter, which is what makes the family swap a .env change - §22.7).
    """
...
def load_settings(
    ...
    return Settings(
        ...
        ingest=IngestSettings(
            llm_title_cleanup=ing["llm_title_cleanup"],
            page_summaries=ing["page_summaries"],
            ...
            max_concurrency=ing["max_concurrency"],
        ),
        eval=EvalSettings(
        ...
        azure_foundry_api_key=(env.get("AZURE_FOUNDRY_API_KEY") or "").strip() or None,
