"""The section tree - four signals in priority order (§14.2, ingest step S6).

⚠️ SEC FILINGS HAVE NO SEMANTIC HEADING TAGS. ✅ MEASURED: only 6 of 78 filings
...
nothing. The four signals, in the order they are trusted:
  ...
  4. FRONT TOC    enrichment ONLY. ⚠️ Its printed page numbers must NEVER be
                  used for location - they index the filing's own pagination,
                  which drifts from our derived index by -2 to +16.

⚠️ EXTRACT ANCHORS WITH lxml, NEVER WITH A REGEX. A regex probe reported
...
# §22.3 - measured 73% gold-page recall from these patterns alone, no LLM.
...
def build_tree(
    ...
    *,
    ...
    An 8-K gets NO tree (Decision B part 1): it has no Items in the 10-K sense,
