"""Ingest orchestration - steps S1..S12 (IMPLEMENTATION_PLAN §21.1).
...
the brief grades a VISIBLE processing indicator on upload, with a 10-minute
budget. Parsing all 78 filings is ~26 s, so the budget is not the constraint -
honest progress reporting is.

Nothing here reads a question, a gold answer or a benchmark id (§25.3 rule 1):
...
def build_lexical_text(
    ...
    """The composite BM25 field (§16.2).

    ⚠️ NOT the summary alone. Query rewriting to exact GAAP names moved BM25
    ...
    *,
    ...
    for page in doc.pages:                                            # S4, S5
        ...
        for order_idx, el in enumerate(table_elements):
            ...
            # Typed cells ONLY where alignment validated - fail closed (§16.3).
