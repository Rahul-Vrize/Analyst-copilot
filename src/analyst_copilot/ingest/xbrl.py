"""Inline XBRL fact extraction - ingest step S8 (IMPLEMENTATION_PLAN §17).

⚠️ XBRL IS A FIRST-CLASS *FACT* PATH, NOT A FIRST-CLASS *ANSWER* PATH.
...
THREE NON-NEGOTIABLE RULES (§17.3), all enforced below:
  ...
  3. THE ROW LABEL IS THE SAFETY NET (§17.4). Walking from the fact element up
...
Pure lxml. No Arelle (D8), no model call, negligible ingest cost.
"""
...
def _local(tag) -> str:
    ...
    ⚠️ TWO SPELLINGS, AND THE HTML ONE IS THE ONE WE ACTUALLY GET.
...
def label_matches_metric(row_label: str | None, metric_terms: list[str]) -> bool:
    """Deterministic wrong-concept detector - no LLM (§17.4 payoff 1).
...
def parse_fact_value(el: etree._Element) -> Decimal | None:
    ...
    ⚠️ BOTH ATTRIBUTES CHANGE THE NUMBER. A filing renders "1,577" with
