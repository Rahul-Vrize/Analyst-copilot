"""JSON Schemas, one per prompt (§22.7 rule 2: every call is structured).

⚠️ THE SCHEMA AND ITS PROMPT MUST DECLARE THE SAME FIELDS. Measured failure:
...
That is §23.3 in miniature: a contradiction between two instructions is
...
QUERY_REWRITE = _obj(
    {
        "query": _STR,
        "gaap_line_items": _STR_LIST,
    }
)

PAGE_SUMMARY = _obj({"summary": _STR})
            ...
            "items": _obj(
                {
                    ...
                    "kind": {
                        "type": "string",
                        "enum": ["part", "item", "note", "statement", "mdna", "exhibit", "other"],
                    },
                }
            ),
        }
...
BY_STAGE: dict[str, tuple[str, dict[str, Any]]] = {
    ...
    "query_rewrite": ("query_rewrite", QUERY_REWRITE),
    "summarizer": ("page_summary", PAGE_SUMMARY),
    "section_titles": ("section_titles", SECTION_TITLES),
