"""FastAPI service (§8, §22.5).

Imports only `query.pipeline` and the schemas - no SQL, no vendor SDK, no
retrieval internals (§24.2). The colleague's UI and the Streamlit client both
bind here, so this is the contract.

The four GRADED product controls map to these endpoints:
