"""Dense retrieval - the third RRF signal (§4, D10).

⚠️ IN MEMORY, NOT A DATABASE QUERY PER QUESTION, and that is a deliberate
...
⚠️ VECTORS ARE NORMALISED ONCE, AT LOAD. Cosine similarity between unit vectors
...
⚠️ A PAGE WITH NO VECTOR MUST NOT RANK. `dense_search` in SQL filters
...
class DenseRetriever:
    ...
    def search(self, query: str, scope: list[str], k: int = 20) -> list[Hit]:
        ...
        ⚠️ NEVER CORPUS-WIDE. §4 requires every retriever to be scoped to the
        routed candidates: corpus-wide similarity search is the shared-store
