...
_QUESTION_HINTS: list[tuple[re.Pattern[str], str]] = [
    ...
    # ⚠️ WIDENED FROM A MEASURED FAILURE. financebench_id_01865 asks "excluding
...
class AnchorRetriever:
    ...
    # ⚠️ MD&A IS NOT A TWO-PAGE STATEMENT, AND THE ANCHOR RULE ASSUMED IT WAS.
    ...
    def _add_narrative_spans(self, spans, pages_by_doc) -> None:
        ...
        for span in spans:
            ...
            # ⚠️ A SEPARATE BUCKET, NOT MERGED INTO `mdna`, AND THE
    ...
    # ⚠️ PAGE ORDER IS NOT RELEVANCE ORDER, AND FOR A 26-PAGE BUCKET THAT
    ...
    def search(self, query: str, scope: list[str], k: int = 40) -> list[Hit]:
        ...
        for h in hits:
            if h.page_id not in best or h.score > best[h.page_id].score:
                best[h.page_id] = h
        # ⚠️ TIES BREAK BY ROUTER RANK, NOT ALPHABETICALLY. `k` is a budget over
